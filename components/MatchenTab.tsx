...
export default function MatchenTab({
  ...
  filterRonde, setFilterRonde // <-- Ontvangt nu de filter opties
}: any) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      {/* DE STRAKKE FILTER SCHAKELAAR */}
      <div style={{ display: 'flex', background: 'rgba(255,255,255,0.5)', borderRadius: '12px', padding: '4px', border: '2px solid #E9ECEF', marginBottom: '15px' }}>
        <button 
          onClick={() => setFilterRonde('Alle')} 
          style={{ 
            flex: 1, padding: '12px', borderRadius: '10px', border: 'none', 
            background: filterRonde === 'Alle' ? '#FFF' : 'transparent', 
            fontWeight: 900, color: filterRonde === 'Alle' ? 'var(--crayola)' : '#6C757D', 
            boxShadow: filterRonde === 'Alle' ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', 
        ...
        <button 
          onClick={() => setFilterRonde('Nog in te vullen')} 
          style={{ 
            flex: 1, padding: '12px', borderRadius: '10px', border: 'none', 
            background: filterRonde === 'Nog in te vullen' ? '#FFF' : 'transparent', 
            fontWeight: 900, color: filterRonde === 'Nog in te vullen' ? 'var(--rose)' : '#6C757D', 
            boxShadow: filterRonde === 'Nog in te vullen' ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', 
            cursor: 'pointer', transition: '0.2s', textTransform: 'uppercase', letterSpacing: '0.5px', fontSize: '0.75rem' 
          }}
        >
          ⚠️ Nog in te vullen
      ...
      {/* MATCHEN LIJST OF SUCCES-MELDING */}
      {gefilterdeMatchen.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px 20px', background: 'rgba(255,255,255,0.8)', borderRadius: '16px', border: '2px dashed #ADB5BD', marginTop: '10px' }}>
          <div style={{ fontSize: '3rem', marginBottom: '10px' }}>✅</div>
          <p style={{ fontWeight: 900, color: '#111827', fontSize: '1.2rem', margin: 0 }}>Alles is ingevuld!</p>
          <p style={{ color: '#6C757D', fontSize: '0.8rem', fontWeight: 800, marginTop: '5px' }}>
            {filterRonde === 'Nog in te vullen' 
              ? "Je bent helemaal bij met je pronostieken. Goed bezig!" 
              : "Er staan momenteel geen matchen op het programma."}
          </p>
        </div>
      ) : (
