...
export default function RankingTab({ klassement, actieveSpeler, toggleBetaald }: any) {
  ...
  // Schakelaar status (standaard FALSE = Zonder bonus zien)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
      ...
      <style>{`
        ...
        .pt-tag { font-size: 0.6rem; font-weight: 900; padding: 3px 8px; border-radius: 6px; text-transform: uppercase; transition: 0.3s; }
        ...
        /* Highlight de tag die momenteel actief is in het klassement */
        .pt-active { background: var(--crayola); color: #FFF; border-color: var(--crayola); }
        .pt-bonus-active { background: var(--magenta); color: #FFF; border-color: var(--magenta); }
      ...
      <div style={{ display: 'flex', background: 'rgba(255,255,255,0.5)', borderRadius: '12px', padding: '4px', border: '2px solid #E9ECEF' }}>
        <button 
          ...
          style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', background: !toonBonus ? '#FFF' : 'transparent', fontWeight: 900, color: !toonBonus ? 'var(--crayola)' : '#6C757D', boxShadow: !toonBonus ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', cursor: 'pointer', transition: '0.2s', textTransform: 'uppercase', letterSpacing: '0.5px' }}
        >
          ⚽ Zonder Bonus
        </button>
        <button 
          onClick={() => setToonBonus(true)} 
          style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', background: toonBonus ? '#FFF' : 'transparent', fontWeight: 900, color: toonBonus ? 'var(--magenta)' : '#6C757D', boxShadow: toonBonus ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', cursor: 'pointer', transition: '0.2s', textTransform: 'uppercase', letterSpacing: '0.5px' }}
        >
          💎 Met Live Bonus
        </button>
      </div>
          ...
          <div key={s.id} className={`rank-card ${podiumClass}`} style={{ border: isMij && i > 2 ? '2px solid var(--crayola)' : '' }}>
            ...
            <div className="rank-info">
              ...
              <div className="points-breakdown">
                ...
                <span className={`pt-tag ${toonBonus ? 'pt-bonus-active' : 'pt-bonus'}`}>💎 {s.bonus_score || 0} Bonus</span>
              </div>

              <div className="rank-stats">
                <span>{s.exact} Exact</span>
                <span>•</span>
                <span>{s.winnaarCorrect} Winnaar</span>
              </div>
