...
export default function Home() {
  ...
  const ontgrendel = async (e: React.FormEvent) => {
    ...
    // 1. Zoek eerst de speler op naam
    ...
    if (error || !speler) {
      setStatus('Naam niet gevonden.');
      return;
    }

    // 2. Vergelijk de code (we maken van beide een string om fouten te voorkomen)
  ...
  return (
    <main className="main-container">
      <style>{`
        ...
        .main-container {
          margin: 0; padding: 15px 15px 80px 15px; min-height: 100vh; display: flex; flex-direction: column; align-items: center;
          font-family: 'Inter', -apple-system, system-ui, sans-serif;
          background: linear-gradient(-45deg, #F3C98B, #DAA588, #C46D5E, #F56960);
          background-size: 400% 400%; animation: gradientBG 15s ease infinite; color: white; box-sizing: border-box;
        }
        @keyframes gradientBG { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
        
        .install-guide {
          background: rgba(255, 255, 255, 0.08); border-radius: 18px; padding: 12px 18px; font-size: 0.68rem; margin-bottom: 20px; 
          border: 1px solid rgba(255,255,255,0.15); width: 100%; max-width: 420px; text-align: center; color: rgba(255,255,255,0.85);
        }
        .glass-card { 
        ...
        .payment-box {
          background: rgba(255, 255, 255, 0.07); border-radius: 20px; padding: 20px; margin-top: 25px; 
          border: 1px solid rgba(156, 246, 246, 0.2); text-align: left;
        }
        ...
        .label { font-size: 0.6rem; font-weight: 800; text-transform: uppercase; letter-spacing: 1.2px; margin-left: 12px; margin-bottom: 5px; display: block; opacity: 0.7; text-align: left; }
        .input-field { width: 100%; padding: 15px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.06); color: white; font-size: 1rem; outline: none; box-sizing: border-box; margin-bottom: 10px; }
        ...
        .btn-secondary { width: 100%; padding: 14px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.25); background: transparent; color: white; font-weight: 600; font-size: 0.85rem; cursor: pointer; }
        .chip-container { display: flex; flex-wrap: wrap; gap: 8px; justify-content: center; margin-top: 40px; width: 100%; max-width: 420px; }
        .chip { background: rgba(255, 255, 255, 0.06); padding: 7px 16px; border-radius: 50px; border: 1px solid rgba(255, 255, 255, 0.1); font-size: 0.78rem; }
        .chip-active { background: #9CF6F6; color: #1A3C40; border: none; font-weight: 700; }
      `}</style>

      <div className="install-guide">
        <strong style={{color: '#9CF6F6'}}>INSTALLEER ALS APP</strong><br/>
        iPhone: Tik op ⎋ 'Zet op beginscherm' • Android: Tik op ⋮ 'Toevoegen aan startscherm'
      </div>
      ...
      <div className="glass-card">
        ...
        {actieveSpeler ? (
          <div>
            <p style={{ opacity: 0.8, margin: 0 }}>Welkom,</p>
            <h2 style={{ color: '#9CF6F6', fontSize: '2.2rem', margin: '5px 0 20px 0' }}>{actieveSpeler.naam}</h2>
            <button className="btn-secondary" onClick={() => { localStorage.removeItem('wk_speler_id'); setActieveSpeler(null); }}>Ander profiel</button>
          </div>
        ) : (
          <>
            <form onSubmit={ontgrendel}>
              ...
              <input className="input-field" type="text" placeholder="Geheime code" value={invoerCode} onChange={e => setInvoerCode(e.target.value)} />
              <button className="btn-primary" type="submit">LOGIN</button>
            </form>
            <div style={{ margin: '25px 0', display: 'flex', alignItems: 'center', opacity: 0.2 }}><hr style={{ flex: 1 }} /><span style={{ padding: '0 15px', fontSize: '0.6rem', fontWeight: 900 }}>OF</span><hr style={{ flex: 1 }} /></div>
        ...
        {/* BETAALGEGEVENS (Nu altijd duidelijk zichtbaar onderaan de kaart) */}
        <div className="payment-box">
          ...
          <p style={{ margin: 0, fontSize: '0.85rem', opacity: 0.9, lineHeight: '1.4' }}>
            Mededeling: <strong>WK prono + {actieveSpeler ? actieveSpeler.naam : 'Jouw Naam'}</strong>
          </p>
        </div>
